...
/** Shown to a confined user (print / olloi) on any section other than their own. */
export function LockedNotice({
  home = "/content-generator",
  labelEn = "the Content Generator",
  ...
  return (
    <div className="flex flex-col items-center justify-center text-center py-24 px-6 max-w-[520px] mx-auto">
      ...
      <p className="text-[14px] text-[var(--text-3)] leading-relaxed mb-6">
        {t(`Your account has access to ${labelEn} only. The rest of the newsroom isn't available for this login.`,
           `आपके अकाउंट को केवल ${labelHi} का एक्सेस है। बाकी न्यूज़रूम इस लॉगिन के लिए उपलब्ध नहीं है।`)}
      </p>
      <Link href={home}
        className="inline-flex items-center gap-2 text-[13px] font-medium px-4 py-2.5 rounded-lg text-white"
        style={{ background: "var(--purple)" }}>
        {t(`Go to ${labelEn}`, `${labelHi} पर जाएँ`)} <ArrowRight size={15} />
      </Link>
    </div>
  );
}
